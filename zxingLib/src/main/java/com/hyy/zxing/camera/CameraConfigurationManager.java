...
import com.hyy.zxing.Preferences;
import com.hyy.zxing.camera.open.CameraFacing;
import com.hyy.zxing.camera.open.OpenCamera;
...
final class CameraConfigurationManager {
    ...
    void setDesiredCameraParameters(OpenCamera camera, boolean safeMode) {
        ...
        if (parameters == null) {
            LogUtils.w("Device error: no camera parameters are available. Proceeding without " +
                    "configuration.");
            return;
        }
...
//            parameters.setZoom(parameters.getMaxZoom() / 10);
        ...
        com.hyy.zxing.camera.CameraConfigurationUtils.setFocus(parameters,
                prefs.getBoolean(Preferences.KEY_AUTO_FOCUS, true),
                prefs.getBoolean(Preferences.KEY_DISABLE_CONTINUOUS_FOCUS, true), safeMode);

        if (!safeMode) {
            if (prefs.getBoolean(Preferences.KEY_INVERT_SCAN, false)) {
                com.hyy.zxing.camera.CameraConfigurationUtils.setInvertColor(parameters);
            }

            if (!prefs.getBoolean(Preferences.KEY_DISABLE_BARCODE_SCENE_MODE, true)) {
                com.hyy.zxing.camera.CameraConfigurationUtils.setBarcodeSceneMode(parameters);
            }

            if (!prefs.getBoolean(Preferences.KEY_DISABLE_METERING, true)) {
                com.hyy.zxing.camera.CameraConfigurationUtils.setVideoStabilization(parameters);
                com.hyy.zxing.camera.CameraConfigurationUtils.setFocusArea(parameters);
                com.hyy.zxing.camera.CameraConfigurationUtils.setMetering(parameters);
            }
    ...
    private void initializeTorch(Camera.Parameters parameters, SharedPreferences prefs,
        ...
        boolean currentSetting =
                com.hyy.zxing.camera.FrontLightMode.readPref(prefs) == com.hyy.zxing.camera.FrontLightMode.ON;
        doSetTorch(parameters, currentSetting, safeMode);
    }

    private void doSetTorch(Camera.Parameters parameters, boolean newSetting, boolean safeMode) {
        com.hyy.zxing.camera.CameraConfigurationUtils.setTorch(parameters, newSetting);
        SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(context);
        if (!safeMode && !prefs.getBoolean(Preferences.KEY_DISABLE_EXPOSURE, true)) {
            com.hyy.zxing.camera.CameraConfigurationUtils.setBestExposure(parameters, newSetting);
        }
    }
